...
class SentimentPreprocessor:
    ...
    def get_sentiment(self, text):
        ...
        Returns polarity, subjectivity, and categorized sentiment (Positive/Neutral/Negative)
        ...
        return {
            **sentiment_result,
            'analysis_details': {
                ...
                'sentiment_words': sorted(sentiment_words, key=lambda x: abs(x['sentiment']), reverse=True)[:5],
                'sentiment_distribution': {
                    'positive_words': len([w for w in sentiment_words if w['sentiment'] > 0]),
                    'negative_words': len([w for w in sentiment_words if w['sentiment'] < 0]),
                    'neutral_words': len([w for w in sentiment_words if w['sentiment'] == 0])
                }
            }
        }
